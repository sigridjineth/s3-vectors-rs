...
thread_local! {
    static BERT_MODEL: RefCell<Option<Result<Rc<BertModelWrapper>>>> = const { RefCell::new(None) };
}
...
impl BertModelWrapper {
    ...
    fn load_from_files(
        ...
        let model =
            BertModel::load(vb, &config).context("Failed to load BERT model from weights")?;
    ...
    pub fn embed_batch(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        ...
        for chunk in texts.chunks(32) {
            ...
            let token_ids =
                Tensor::new(token_ids_vec, &self.device)?.reshape((batch_size, max_len))?;
