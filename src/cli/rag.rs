...
pub enum RagSubcommands {
    ...
    Init {
        #[arg(
            ...
            default_value = "rag-vectors-default"
        )]
        bucket: String,
        ...
        #[arg(
            ...
            default_value = "documents-default"
        )]
        index: String,
    },
...
impl RagCommand {
    pub async fn execute(
        ...
        match &self.command {
            ...
            RagSubcommands::Ingest {
                directory,
                bucket,
                index,
            } => {
                self.ingest_documents(client, directory, bucket, index, output_format)
                    .await
            }
            RagSubcommands::Query {
                query,
                top_k,
                bucket,
                index,
            } => {
                self.query_rag(client, query, *top_k, bucket, index, output_format)
                    .await
            }
            RagSubcommands::Interactive { bucket, index } => {
                self.interactive_query(client, bucket, index, output_format)
                    .await
            }
        }
    }
    ...
    async fn ingest_documents(
        ...
        match self
            .verify_bucket_and_index(client, bucket_name, index_name)
            .await
        {
            Ok(_) => {}
            Err(e) => {
                ...
                eprintln!(
                    "\n💡 {} Run: {}",
                    "Tip:".yellow(),
                    format!("rag init --bucket {bucket_name} --index {index_name}").cyan()
                );
