...
use crate::{CreateIndexRequest, DistanceMetric, ListIndexesResponse, S3VectorsClient};
use anyhow::Result;
...
impl IndexCommand {
    pub async fn execute(
        ...
        match &self.command {
            ...
            IndexSubcommands::Delete {
                bucket,
                name,
                force,
            } => {
                self.delete_index(client, bucket, name, *force, output_format)
                    .await
            }
        }
    }
    ...
    async fn delete_index(
        ...
        if !force {
            ...
            let proceed = Confirm::new()
                .with_prompt(format!(
                    "Are you sure you want to delete index '{bucket}/{name}'?"
                ))
                .default(false)
                .interact()?;
        ...
        match output_format {
            OutputFormat::Table => {
                println!("✓ Index '{bucket}/{name}' deleted successfully");
            }
            _ => {
                let result = serde_json::json!({
                    "status": "success",
