...
impl InstallModelsCommand {
    pub async fn execute(&self) -> Result<()> {
        ...
        for (idx, file) in files_to_download.iter().enumerate() {
            let file_path = model_path.join(file.name);
            println!("\n[{}/{}] Downloading {}...", idx + 1, files_to_download.len(), file.name.cyan());
            
            match self.download_file(&client, file, &file_path, &multi_progress).await {
                Ok(_) => {
                ...
                Err(e) => {
                    if file.required {
                        ...
                        eprintln!("{} Failed to download {} (optional): {}", "⚠".yellow(), file.name, e);
                    }
                }
            }
        }
        ...
        if all_required_present {
            ...
            println!("\nModels installed to: {}", model_path.display().to_string().cyan());
