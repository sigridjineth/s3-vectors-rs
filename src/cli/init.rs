...
impl InitCommand {
            ...
            if self.test_credentials(&client).await {
                ...
                match save_option {
                    SaveOption::AwsCredentials(profile_name) => {
                        ...
                        println!("\n{} Configuration saved to ~/.aws/credentials", "✓".green());
                        println!("You can now use S3 Vectors with: {}", 
                            format!("s3-vectors --profile {}", profile_name).cyan());
                    },
                    SaveOption::Environment => {
                        self.show_export_commands(&access_key_id, &secret_access_key, session_token.as_deref(), &region);
                    },
                    SaveOption::NoSave => {
                ...
                println!("{} Authentication failed. Please check your credentials.", "✗".red());
    ...
    async fn setup_profile(&self) -> Result<Option<S3VectorsClient>> {
        ...
        if !creds_path.exists() {
            println!("\n{} No AWS credentials file found at: {:?}", "⚠".yellow(), creds_path);
        ...
        if profiles.is_empty() {
            println!("\n{} No profiles found in AWS credentials file.", "⚠".yellow());
            return self.setup_access_keys().await;
        }
    ...
    fn save_to_aws_credentials(
        ...
        if !aws_dir.exists() {
            fs::create_dir_all(&aws_dir)
                .context("Failed to create .aws directory")?;
        }
    ...
    fn show_export_commands(&self, access_key: &str, secret_key: &str, session_token: Option<&str>, region: &str) {
