...
impl S3VectorsClient {
    ...
    async fn execute_request<T: DeserializeOwned>(
        ...
        loop {
            ...
            // Sign the request
            let headers = signer
                .sign_request("POST", &url, HashMap::new(), &body_bytes)
                .await?;
                ...
                match status {
                    ...
                    StatusCode::CONFLICT => {
                        if let Some(error_type) = service_error.error_type {
                            if error_type.contains("ConflictException")
                                || error_type.contains("AlreadyExistsException")
                            {
    ...
    pub async fn create_vector_bucket(
        ...
        validate_bucket_name(bucket_name).map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        ...
        let _: CreateVectorBucketResponse = self
            .execute_request("/CreateVectorBucket", Some(request))
            .await?;
    ...
    pub async fn delete_vector_bucket(&self, bucket_name: &str) -> Result<(), S3VectorsError> {
        ...
        self.execute_request::<serde_json::Value>("/DeleteVectorBucket", Some(request))
            .await?;
    ...
    pub async fn list_vector_buckets(
        ...
        self.execute_request("/ListVectorBuckets", Some(request))
            .await
    }

    #[tracing::instrument(skip(self), fields(bucket_name))]
    pub async fn describe_vector_bucket(
        &self,
        bucket_name: &str,
    ) -> Result<VectorBucket, S3VectorsError> {
        validate_bucket_name(bucket_name).map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        ...
        let response: serde_json::Value = self
            .execute_request("/GetVectorBucket", Some(request))
            .await?;
        ...
        let bucket: VectorBucket = serde_json::from_value(
            response
                .get("vectorBucket")
                .ok_or_else(|| {
                    S3VectorsError::ServiceError("Missing vectorBucket in response".to_string())
                })?
                .clone(),
        )?;
    ...
    pub async fn create_index(&self, request: CreateIndexRequest) -> Result<(), S3VectorsError> {
        ...
        info!(
            "Creating index {} in bucket {}",
            request.index_name, request.vector_bucket_name
        );
        let _: serde_json::Value = self.execute_request("/CreateIndex", Some(request)).await?;
    ...
    pub async fn delete_index(
        ...
        validate_bucket_name(bucket_name).map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        validate_index_name(index_name).map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        ...
        self.execute_request::<serde_json::Value>("/DeleteIndex", Some(request))
            .await?;
    ...
    pub async fn list_indexes(
        ...
        self.execute_request("/ListIndexes", Some(request)).await
    }
    ...
    pub async fn describe_index(
        ...
        bucket_name: &str,
        index_name: &str,
    ) -> Result<VectorIndex, S3VectorsError> {
        validate_bucket_name(bucket_name).map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        validate_index_name(index_name).map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        ...
        let response: serde_json::Value = self.execute_request("/GetIndex", Some(request)).await?;
        ...
        let index: VectorIndex = serde_json::from_value(
            response
                .get("index")
                .ok_or_else(|| {
                    S3VectorsError::ServiceError("Missing index in response".to_string())
                })?
                .clone(),
        )?;
    ...
    pub async fn put_vectors(&self, request: PutVectorsRequest) -> Result<(), S3VectorsError> {
        ...
        if request.vectors.is_empty() {
            return Err(S3VectorsError::Validation(
        ...
        info!(
            "Putting {} vectors to index {} in bucket {}",
            request.vectors.len(),
            request.index_name,
            request.vector_bucket_name
        );

        self.execute_request::<serde_json::Value>("/PutVectors", Some(request))
            .await?;
    ...
    pub async fn get_vectors(
    ...
    ) -> Result<GetVectorsResponse, S3VectorsError> {
        validate_bucket_name(&request.vector_bucket_name)
            .map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        validate_index_name(&request.index_name)
            .map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        ...
        info!(
            "Getting {} vectors from index {} in bucket {}",
            request.keys.len(),
            request.index_name,
            request.vector_bucket_name
        );

        self.execute_request("/GetVectors", Some(request)).await
    }
    ...
    pub async fn delete_vectors(
        ...
        info!(
            "Deleting {} vectors from index {} in bucket {}",
            request.keys.len(),
            request.index_name,
            request.vector_bucket_name
        );

        self.execute_request::<serde_json::Value>("/DeleteVectors", Some(request))
            .await?;
    ...
    pub async fn list_vectors(
        ...
        info!(
            "Listing vectors in index {} of bucket {}",
            request.index_name, request.vector_bucket_name
        );

        self.execute_request("/ListVectors", Some(request)).await
    }

    pub async fn query_vectors(
        &self,
        request: QueryVectorsRequest,
    ) -> Result<QueryVectorsResponse, S3VectorsError> {
        validate_bucket_name(&request.vector_bucket_name)
            .map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        validate_index_name(&request.index_name)
            .map_err(|e| S3VectorsError::Validation(e.to_string()))?;
        ...
        info!(
            "Querying vectors in index {} of bucket {}",
            request.index_name, request.vector_bucket_name
        );

        self.execute_request("/QueryVectors", Some(request)).await
    }
    ...
    pub async fn get_index(
        ...
        index_name: &str,
    ) -> Result<GetIndexResponse, S3VectorsError> {
        ...
        info!(
            "Getting index {} in bucket {}",
            index_name, vector_bucket_name
        );

        self.execute_request("/GetIndex", Some(request)).await
    }
    ...
    pub async fn put_vector_bucket_policy(
        ...
        self.execute_request::<serde_json::Value>("/PutVectorBucketPolicy", Some(request))
            .await?;
    ...
    pub async fn get_vector_bucket_policy(
        ...
        let response: PolicyResponse = self
            .execute_request("/GetVectorBucketPolicy", Some(request))
            .await?;
    ...
    pub async fn delete_vector_bucket_policy(
        ...
        self.execute_request::<serde_json::Value>("/DeleteVectorBucketPolicy", Some(request))
            .await?;
        ...
        Err(S3VectorsError::NotFound(_)) => {
            ...
            match client.create_vector_bucket(bucket_name).await {
                ...
                Err(S3VectorsError::AlreadyExists(_)) => {
                    ...
                    info!(
                        "Bucket {} was created by another process, describing existing",
                        bucket_name
                    );
...
async fn wait_for_bucket_active(
    ...
    for _ in 0..60 {
        ...
        match bucket.status {
            ...
            None => {
                info!(
                    "Bucket {} status is None, assuming in-progress",
                    bucket_name
                );
                sleep(Duration::from_secs(1)).await;
            }
    ...
    Err(anyhow::anyhow!(
        "Timeout waiting for bucket to become active"
    ))
}
